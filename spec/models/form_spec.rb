...
RSpec.describe Form, type: :model do
  ...
  describe '商品購入' do
    context '保存できる場合' do
      ...
      it '建物名が無くてもその他の条件が適切であれば登録ができる' do
        @form.building = ''
        expect(@form).to be_valid
      end
    end
    context '保存できない場合' do
      ...
      it '郵便番号が半角数字のみでは登録できない' do
        @form.postal_code = Faker::Number.number(digits: 7)
        @form.valid?
        expect(@form.errors.full_messages).to include('Postal code は"-"を含んだ半角数字で入力してください')
      end
      it '郵便番号が半角数字以外では登録できない' do
        @form.postal_code = Faker::Lorem.characters(number: 7).to_s.insert(3, '-')
        @form.valid?
        expect(@form.errors.full_messages).to include('Postal code は"-"を含んだ半角数字で入力してください')
      end
      it '郵便番号が9桁以上では登録できない' do
        @form.postal_code = Faker::Number.number(digits: 8).to_s.insert(3, '-')
        @form.valid?
